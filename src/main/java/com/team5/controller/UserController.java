...
public class UserController {
    ...
    public ModelAndView goSignUp(){return new ModelAndView("signUp");}

    /**
     * 用户注册
     * @param user 用户在前端提交的注册信息
     *
     * */
    @RequestMapping(value = "/signUp",method = RequestMethod.POST)
    @ResponseBody
    public Result doSignUp(@RequestBody User user, HttpServletRequest request) {
        // userService验证是否学号已经存在
        boolean flag = userService.checkExistSid(user.getId());
        log.info("request: user/signUp , user: " + user.toString());
        if (!flag) {
            int creatFlag = userService.creatUser(user);
            return ResultGenerator.genSuccessResult("注册成功，请重新登录,id为"+creatFlag);
        }else {
